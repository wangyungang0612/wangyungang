...
public class ChartView extends View {
    ...
    public int YScale = 13;// Y的刻度长度
    public int XLength = 850; // X轴的长度
    public int YLength = 450; // Y轴的长度
    public List<String> XLabel; // X的刻度
    public List<String> YLabel; // Y的刻度
    public List<String> Data; // 数据
    public String Title ="扫码成功率"; // 显示的标题
...
//    }
    ...
    public void SetInfo(List<String> allSaoMaTimeList, List<String> YLabels, List<String> allSaoMaList,String strTitle
    ) {
        XLabel = allSaoMaTimeList;
        YLabel = YLabels;
        Data = allSaoMaList;
        Title = strTitle;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i * YScale < YLength; i++) {
            if(i%5==0){
                canvas.drawLine(XPoint, YPoint - i * YScale, XPoint + 6, YPoint - i
                        * YScale, paint); // 刻度 XPoint+5画出了一条短的小横线
            }else {
                canvas.drawLine(XPoint, YPoint - i * YScale, XPoint + 2, YPoint - i
                        * YScale, paint); // 刻度 XPoint+5画出了一条短的小横线
            }

            try {
                canvas.drawText(YLabel.get(i), XPoint - 30,
                        YPoint - i * YScale -8, paint); // 文字
            } catch (Exception e) {
            }
        }
        ...
        for (int i = 0; i * XScale < XLength; i++) {
            ...
            try {
                canvas.drawText(XLabel.get(i), XPoint + i * XScale - 20,
                        YPoint + 20, paint); // 文字
                // 数据值
                if (i > 0 && YCoord(Integer.parseInt(Data.get(i-1))) != -999
                        && YCoord(Integer.parseInt(Data.get(i))) != -999) // 保证有效数据
                    canvas.drawLine(XPoint + (i - 1) * XScale,
                            YCoord(Integer.parseInt(Data.get(i-1))), XPoint + i * XScale,
                            YCoord(Integer.parseInt(Data.get(i))), paint);
                canvas.drawCircle(XPoint + i * XScale, YCoord(Integer.parseInt(Data.get(i))), 2,
                        paint);
            } catch (Exception e) {
            }
        }
    ...
    private int YCoord(int y0) // 计算绘制时的Y坐标，无数据时返回-999
    {
        ...
        try {
            if(y<50){
                return YPoint - (y)* YScale / 10;
            }else {
                return YPoint - (y-40)* YScale / 2; //Integer.parseInt(YLabel.get(1) 此处应该是除以y轴的第一个数字，因为这里是空也就是2 故除以2
            }
